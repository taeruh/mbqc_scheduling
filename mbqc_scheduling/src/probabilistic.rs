//! Probabilistic accept functions (a little bit like in Markov chains), for the search
//! algorithm.
...
/// Boxed version of [Accept].
pub type AcceptBox = Box<Accept>;
...
#[derive(Clone, Copy)]
/// Parameters for the [AcceptFunc::ParametrizedHeavyside] accept function. Compare the
/// documented function there, where `param` is this parameter struct.
...
/// [Accept], which describes the signature of the underlying accept function.
#[derive(Default)]
pub enum AcceptFunc {
    #[default]
    /// A fixed accept function that is used by default. Following the definitions in
    /// [Accept], this function is defined as
    /// ```ignore
    /// # fn builtin_heavyside(_: f64, minimal_mem: f64, last_max_mem: f64, _: f64,
    /// # cur_mem: f64, num_remaining_nodes: f64, num_total_nodes: f64,) -> f64 {
    /// let diff = minimal_mem - f64::max(cur_mem, last_max_mem);
    /// if diff < 0. {
    ///     0.
    /// } else {
    ...
    /// Following [Accept], this function is defined as (`param` is of type
    /// [HeavysideParameters])
    /// ```ignore
    /// # fn create_parametrized_heavyside(param: HeavysideParameters,)
    /// # -> impl Fn(f64, f64, f64, f64, f64, f64, f64) -> f64 {
    /// # move |_, minimal_mem, last_max_mem, _, cur_mem, num_remaining_nodes,
    /// # num_total_nodes| {
    /// let diff = minimal_mem - f64::max(cur_mem, last_max_mem);
    /// if diff < param.cutoff {
    ///     0.
